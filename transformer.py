        ...
        for epoch in range(epochs):
            ...
            if X_val is not None and y_val is not None:
                ...
                if avg_val_loss < best_val_loss:
                    ...
                    if lr_reduction_counter >= lr_patience:
                        for param_group in self.optimizer.param_groups:
                            param_group["lr"] = max(
                                param_group["lr"] * lr_factor, min_lr
                            )
                        lr_reduction_counter = 0
                        if verbose > 0:
                            print(
                                f"Reduced learning rate to {self.optimizer.param_groups[0]['lr']}"
                            )
                    ...
                    if patience_counter >= patience:
                        ...
                        if best_model_state:
                            ...
                            self.optimizer.load_state_dict(
                                best_model_state["optimizer_state_dict"]
                            )
                        break
                ...
                if verbose > 0 and ((epoch + 1) % 10) == 0:
                    print(
                        f"Epoch {epoch+1}/{epochs} - Train Loss: {avg_train_loss:.4f} - Train MAE: {avg_train_mae:.4f} - Val Loss: {avg_val_loss:.4f} - Val MAE: {avg_val_mae:.4f}"
                    )
            else:
                # Print train-only results
                if verbose > 0 and ((epoch + 1) % 10) == 0:
                    print(
                        f"Epoch {epoch+1}/{epochs} - Train Loss: {avg_train_loss:.4f} - Train MAE: {avg_train_mae:.4f}"
                    )
