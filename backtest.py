...
class PyTorchBacktestFramework:
    def __init__(
        ...
        self.max_stock_id = max(
            train_test_dict["train_si"].max(), train_test_dict["test_si"].max()
        )
    ...
    def prepare_time_series_batch(
        self, stock_ids, day_indices, features_map, window_size
    ):
        """
        ...
        if valid_sequences:
            sequence_features = torch.tensor(
                np.stack(valid_sequences), dtype=torch.float32
            )
            # Free memory
            del valid_sequences
            gc.collect()
    ...
    def run_backtest(
        ...
        for day_idx in range(np.min(self.train_test_dict["train_di"]), end_day_idx + 1):
            features, stock_ids, returns = self.prepare_data_for_day(
                day_idx, is_training=day_idx < self.test_date
            )
        ...
        if not disable_retraining:
            ...
            print(
                f"Initial training completed in {init_train_mins} min {init_train_secs} sec"
            )
        ...
        for i, day_idx in enumerate(test_days):
                ...
                if retrain_success:
                    ...
                    try:
                        if hasattr(model, "save_model"):
                            checkpoint_path = (
                                f"{self.model_type}_checkpoint_day_{day_idx}.pt"
                            )
            ...
            current_features, current_stock_ids, actual_returns = stock_data_by_day[
                day_idx
            ]
            ...
            if day_idx - last_rebalance_day >= self.rebalance_freq:
                ...
                if self.model_type == "transformer":
                    ...
                    predicted_returns = self.predict_with_regression(
                        model, current_features, current_stock_ids
                    )
                ...
                if total_prediction_count > 0:
                    current_accuracy = (
                        correct_direction_count / total_prediction_count * 100
                    )
                    print(
                        f"Current directional accuracy: {current_accuracy:.2f}% ({correct_direction_count}/{total_prediction_count})"
                    )
                ...
                for col_idx, hist_day in enumerate(range(day_idx - 60, day_idx)):
                    if hist_day in stock_data_by_day:
                        ...
                        valid_indices1 = np.where(
                            hist_stock_ids
                            <= min(self.max_stock_id, len(stock_row_lookup))
                        )[0]
                        valid_indices2 = np.where(
                            stock_row_lookup[hist_stock_ids] >= 0
                        )[0]
                ...
                historical_returns[missing_data_mask] = np.random.normal(
                    0, 0.001, np.sum(missing_data_mask)
                )
                ...
                print(
                    "Return",
                    current_expected_returns.sum(),
                    (current_expected_returns < 0).sum(),
                )
                ...
                if hasattr(optimizer, "mean_variance_optimization"):
                    optimization_result = optimizer.mean_variance_optimization(
                        portfolio_values[-1], current_expected_returns, cov_matrix
                    )
                    ...
                    optimization_result = optimizer.optimize_large_portfolio(
                        current_expected_returns, cov_matrix
                    )
                    optimal_weights = optimization_result["weights"]
                else:
                ...
                if not np.any(np.isnan(optimal_weights)):
                    ...
                    current_weights = dict(
                        zip(
                            unique_current_stocks,
                            [
                                i / len(unique_current_stocks)
                                for i in range(len(unique_current_stocks))
                            ],
                        )
                    )
                ...
                if hasattr(optimizer, "compute_portfolio_metrics"):
                    portfolio_metrics = optimizer.compute_portfolio_metrics(
                        ...
                        np.eye(
                            len(optimal_weights)
                        ),  # Use identity matrix to avoid copying cov_matrix
                    )
                else:
                    # Basic metrics if no compute_portfolio_metrics method available
                    portfolio_metrics = {
                        "expected_return": np.sum(
                            optimal_weights * current_expected_returns
                        ),
                        "volatility": np.sqrt(
                            np.sum(optimal_weights**2)
                        ),  # Simplified using identity matrix
                    }
                ...
                print(
                    f"Portfolio optimization completed in {optimize_mins} min {optimize_secs} sec"
                )
        ...
        print(
            f"Initial Training: {int(initial_training_time // 60)} min {int(initial_training_time % 60)} sec"
        )
        ...
        if total_prediction_count > 0:
            ...
            print(
                f"Directional Accuracy: {final_accuracy:.2f}% ({correct_direction_count}/{total_prediction_count})"
            )
    ...
    def _prepare_time_series_data_batched(
        self,
        start_day,
        end_day,
        stock_ids,
        ...
        for batch_idx in range(n_batch):
            ...
            for stock_id, stock_df in df.groupby("stock_id"):
                ...
                for i in range(batch_start, batch_end):
                    ...
                    if np.any(window_indices >= len(features)) or target_idx >= len(
                        returns
                    ):
                        continue
    ...
    def collect_and_prepare_training_data(
        self,
        model,
        model_type,
        start_day,
        ...
        for train_day in train_days:
            ...
            features, stock_ids, returns = self.prepare_data_for_day(
                train_day, is_training=is_training
            )
        ...
        if model_type == "stock_kernel":
            ...
            for X_train, y_train in self._prepare_time_series_data_batched(
                ...
                if len(X_train) > 10000:
                    print(
                        f"Sampling {10000} sequences from {len(X_train)} for faster training..."
                    )
    ...
    def predict_with_efficient_gpu(
        self,
        current_stock_ids,
        day_idx,
        features_map,
    ...
    def plot_backtest_results(
        ...
        if weights_history:
            ...
            top_stocks = sorted(
                avg_weights.keys(), key=lambda x: avg_weights[x], reverse=True
            )[:10]
            ...
            if metrics_history:
                ...
                if len(metric_days) == len(risk_aversions) == len(market_vols):
                    ax3.plot(
                        metric_days, risk_aversions, label="Risk Aversion", linewidth=2
                    )
                    # Scale market volatility for better visualization
                    ax3.plot(
                        metric_days,
                        np.array(market_vols) * 20,
                ...
                try:
                    ...
                    for m in metrics_history:
                        if "expected_return" in m["metrics"]:
                            ...
                            weights = (
                                list(m["weights"].values()) if "weights" in m else [1.0]
                            )
                    ...
                    for i in range(len(metric_days) - 1):
                        ...
                        if start_idx < end_idx:
                            ...
                            aligned_realized.append(
                                np.mean(period_returns)
                    ...
                    if min_length > 1:
                        ...
                        x_range = np.linspace(
                            min(expected_returns), max(expected_returns), 100
                        )
                        ...
                        correlation = np.corrcoef(expected_returns, aligned_realized)[
                            0, 1
                        ]
                        ax4.text(
                            0.05,
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(self.output_dir, f"backtest_results_{model_name}.png"), dpi=300
        )
        plt.close(fig)
