...
class PyTorchFeatureEngineering:
    def __init__(
        ...
        if stock_count is not None:
            ...
            print(
                f"Shape of x_data: {self.x_data.shape}, Shape of raw_data: {self.raw_data.shape}, Max stock index: {self.si.max()}"
            )
            print(f"From day {self.di.min()} to day {self.di.max()}")
        ...
        self.sector_encoder = OneHotEncoder(
    ...
    def _calculate_nan_statistics(self):
        ...
        if nan_in_raw:
            ...
            for i, col in enumerate(worst_cols_raw):
                print(
                    f"    {self.list_of_data[col]}: {nan_pct_by_col_raw[col]:.2f}% NaNs"
                )
    ...
    def train_test_split(self, split_date_idx=4400):
        ...
        for i, feature_name in enumerate(self.list_of_data):
            train_test_dict[f"train_{feature_name}"] = imputed_raw_data[
                train_indices, i
            ]
        ...
        if self.sector is not None:
            ...
            train_sector_mask = (
                train_test_dict["train_sector"].astype(np.int32) == sector_value
            )
            ...
            for i, feature_name in enumerate(self.list_of_data):
                filtered_train_test_dict[f"train_{feature_name}"] = train_test_dict[
                    f"train_{feature_name}"
                ][train_sector_indices]
                filtered_train_test_dict[f"test_{feature_name}"] = train_test_dict[
        ...
        if self.industry is not None:
            ...
            for i, feature_name in enumerate(self.list_of_data):
                filtered_train_test_dict[f"train_{feature_name}"] = train_test_dict[
                    f"train_{feature_name}"
                ][train_industry_indices]
                filtered_train_test_dict[f"test_{feature_name}"] = train_test_dict[
                    f"test_{feature_name}"
                ][test_industry_indices]
    ...
    def build_autoencoder(
        self, input_dim, encoding_dim=50, hidden_dim=128, dropout_rate=0.2
    ):
        """
    ...
    def extract_alpha_embeddings(
        ...
        if load_path:
            ...
            if os.path.exists(encoder_path) and os.path.exists(scaler_path):
                ...
                # Convert to tensors
                train_tensor = torch.tensor(scaled_train_data, dtype=torch.float32).to(
                    self.device
                )
                test_tensor = torch.tensor(scaled_test_data, dtype=torch.float32).to(
                    self.device
                )
                ...
                with torch.no_grad():
                    train_embeddings = (
                        self.autoencoder.encode(train_tensor).cpu().numpy()
                    )
    ...
    def calculate_technical_indicators(
        ...
        for window in window_sizes:
            ...
            ma_col = f"ma_{window}"
            df[ma_col] = grouped["close"].transform(
            ...
            df[vol_ma_col] = grouped["volume"].transform(
                lambda x: x.rolling(window=window, min_periods=1).mean()
            )
        ...
        for window in window_sizes:
            ...
            df = df.drop(
                ["price_delta", "gain", "loss", "avg_gain", "avg_loss", "rs"], axis=1
            )
        ...
        df["hc"] = (
            df["high"] - df.groupby("stock_id")["close"].shift().fillna(df["high"])
        ).abs()
        df["lc"] = (
            df["low"] - df.groupby("stock_id")["close"].shift().fillna(df["low"])
        ).abs()
        ...
        for window in window_sizes:
            ...
            df[atr_col] = df.groupby("stock_id")["tr"].transform(
                lambda x: x.rolling(window=window, min_periods=1).mean()
            )
        ...
        for window in window_sizes:
            ...
            for stock_id, group in grouped:
                ...
                if len(group_close) <= 1:
                    ...
                    log_returns = log_returns.replace([np.inf, -np.inf], np.nan).fillna(
                        0
                    )
            ...
            if all_volatilities:
                ...
                df = pd.merge(
                    df,
                    vol_df.rename(columns={"vol": vol_col}),
                    on=["stock_id", "day_idx"],
                    how="left",
                )
        ...
        if total_nans > 0:
            ...
            print(
                "No NaNs detected in technical indicators - calculation was successful"
            )
        ...
        if final_nan_count > 0:
            print(
                f"Warning: {final_nan_count} NaNs introduced after merging back to original order"
            )
        ...
        if np.isnan(result).any():
            ...
            print(
                "No NaNs in final output - technical indicators successfully calculated"
            )
    ...
    def encode_categorical_features(
        ...
        train_sector_data,
        train_industry_data,
        test_sector_data=None,
        test_industry_data=None,
    ):
        """
        ...
        is_test_provided = (
            test_sector_data is not None and test_industry_data is not None
        )
        ...
        if is_test_provided:
            ...
            all_industry_data = np.vstack(
                [train_industry_reshaped, test_industry_reshaped]
            )

            # Fit on all data to ensure consistent categories
            print(
                "Fitting encoders on combined train+test data to ensure consistent features"
            )
            self.sector_encoder = OneHotEncoder(
                sparse_output=False, handle_unknown="ignore"
            )
            ...
            train_encoded_industry = self.industry_encoder.transform(
                train_industry_reshaped
            )
            test_encoded_sector = self.sector_encoder.transform(test_sector_reshaped)
            test_encoded_industry = self.industry_encoder.transform(
                test_industry_reshaped
            )

            # Verify shapes match
            print(
                f"Encoded sector shapes - Train: {train_encoded_sector.shape}, Test: {test_encoded_sector.shape}"
            )
            print(
                f"Encoded industry shapes - Train: {train_encoded_industry.shape}, Test: {test_encoded_industry.shape}"
            )

            return (
                train_encoded_sector,
                train_encoded_industry,
                test_encoded_sector,
            ...
            train_encoded_sector = self.sector_encoder.fit_transform(
                train_sector_reshaped
            )
            train_encoded_industry = self.industry_encoder.fit_transform(
                train_industry_reshaped
            )
    ...
    def generate_features(
        self, window_sizes=[5, 10, 20, 50], encoding_dim=50, output_dir=None
    ):
        """
        ...
        train_spx_vix = np.column_stack(
            (train_test_dict["train_ret1_SPX"], train_test_dict["train_close_VIX"])
        )
        test_spx_vix = np.column_stack(
            (train_test_dict["test_ret1_SPX"], train_test_dict["test_close_VIX"])
        )
        ...
        train_ann_features = np.nan_to_num(
            train_ann_features, nan=30
        ...
        if train_features.shape[1] != test_features.shape[1]:
            ...
            if train_features.shape[1] > test_features.shape[1]:
                ...
                print(
                    f"Expanding test features with {diff} zero columns to match training dimensions"
                )
                test_features = np.hstack(
                    [test_features, np.zeros((test_features.shape[0], diff))]
                )
            else:
                diff = test_features.shape[1] - train_features.shape[1]
                print(
                    f"Expanding training features with {diff} zero columns to match test dimensions"
                )
                train_features = np.hstack(
                    [train_features, np.zeros((train_features.shape[0], diff))]
                )
        ...
        print(f"Testing features shape: {test_features.shape}")
        print(
            f"Feature consistency verified: {train_features.shape[1] == test_features.shape[1]}"
        )
